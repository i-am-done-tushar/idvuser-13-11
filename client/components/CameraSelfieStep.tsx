...
export default function CameraCapture({ userId, onStepComplete }: CameraCaptureProps) {
...
const [direction, setDirection] = useState<'left'|'right'|'up'|'down'|'forward'>();
...
const [headSegmentSecondsRecorded, setHeadSegmentSecondsRecorded] = useState(0);
const [mediaRecorder, setMediaRecorder] = useState<MediaRecorder | null>(null);
// const [stream, setStream] = useState<MediaStream | null>(null);
const [isSegmentValid, setIsSegmentValid] = useState(true);
const [verificationDoneForSegment, setVerificationDoneForSegment] = useState<{[key:number]:boolean}>({});
const [headTurnAttemptsPerSegment, setHeadTurnAttemptsPerSegment] = useState<{[key:number]:number}>({});
const [headVerificationCountPerSegment, setHeadVerificationCountPerSegment] = useState<{[key:number]:number}>({});
const [partialSegmentBlobsPerSegment, setPartialSegmentBlobsPerSegment] = useState<{[key:number]:any[]}>({});
const [recordedChunksPerSegment, setRecordedChunksPerSegment] = useState<{[key:number]:Blob[]}>({});
...
const [firstVerificationDirection, setFirstVerificationDirection] = useState<'up'|'down'|'left'|'right'|'forward'|null>(null);
const [secondVerificationDirection, setSecondVerificationDirection] = useState<'up'|'down'|'left'|'right'|'forward'|null>(null);
const [thirdVerificationDirection, setThirdVerificationDirection] = useState<'up'|'down'|'left'|'right'|'forward'|null>(null);

const [headTurnRecordingFailed, setHeadTurnRecordingFailed] = useState(false);

const [headTurnDirection, setHeadTurnDirection] = useState<'left' | 'right' | 'up' | 'down' | 'forward'>('forward');
...
const blinkingDirectionRef = useRef<'left' | 'right' | 'up' | 'down' | 'forward' | null>(null);
...
const plannedVerificationDirectionRef = useRef<Record<number, 'up' | 'down' | 'left' | 'right'>>({});
...
const HEAD_VERIFICATION_TIMEOUT_MS = 4500;
    ...
    const init = async () => {
      try {
        ...
        if (err?.name === 'NotAllowedError') {
          setCameraErrorMessage('❌ Camera permission denied. Please allow access and refresh.');
  ...
  console.log(`📊 Segment durations: [${firstVal}s, ${secondVal}s, ${lastVal}s] = ${firstVal + secondVal + lastVal}s total`);
}, [totalDuration]);

const validateCameraAndVideo = useCallback((loop: () => void): boolean => {
  if (!streamRef.current || !videoRef.current) {
    console.log('info', 'Camera off or video element missing; stopping detection loop.');
    return false;
  }
  return true;
}, []);
...
const scheduleNext = useCallback((fn: FrameRequestCallback | (() => void)): void => {
  rafIdRef.current = requestAnimationFrame(fn as FrameRequestCallback);
}, []);



const validateOval = useCallback((loop: () => void): boolean => {
  if (!ovalRef.current) {
    console.log('info', 'Oval guide not initialized yet; scheduling next frame.');
    scheduleNext(loop);
    return false;
  }
  return true;
...
const drawFaceGuideOverlay = useCallback((brightness: number) => {
  ...
  if (isRecording && (timeRemainingRef.current < 6 && timeRemainingRef.current > 0 || isFaceDetected)) {
...
const checkDifferentFace = useCallback(async (): Promise<boolean> => {
  ...
  if (!referenceFaceDescriptorRef.current) {
    console.log('info', '[FaceCheck] No reference descriptor; skipping check.');
  ...
  try {
    ...
    if (detection && detection.descriptor) {
      ...
      if (distance > 0.6) {
        ...
        if (faceMismatchCounterRef.current > 0) {
          console.log('info', `[FaceCheck] Face match restored, counter reset.`);
        }
        faceMismatchCounterRef.current = 0;
        if (differentFaceDetectedRef.current) {
          differentFaceDetectedRef.current = false;
          // Resume if safe (no multi faces, face detected)
          if (mediaRecorderRef.current && mediaRecorderRef.current.state === 'paused' && !multipleFacesDetectedRef.current && isFaceDetectedRef.current) {
            try {
    ...
    console.log('error', `[FaceCheck] ⚠️ Error during face detection: ${err}`);
    faceMismatchCounterRef.current = 0;
  }
...
const getRandomDirection = (exclude: Array<'up' | 'down' | 'left' | 'right' | 'forward'>): 'up' | 'down' | 'left' | 'right' | 'forward' => {
  const types: ('up' | 'down' | 'left' | 'right' | 'forward')[] = ['up', 'down', 'left', 'right', 'forward'];
  const filtered = types.filter(t => !exclude.includes(t));
  return filtered[Math.floor(Math.random() * filtered.length)];
};


const startBlinking = useCallback((direction: 'left' | 'right' | 'up' | 'down' | 'forward') => {
...
const runVerification = useCallback((direction: 'left' | 'right' | 'up' | 'down' | 'forward'): Promise<boolean> => {
  return new Promise((resolve) => {
    try {
      ...
      try {
        if (!streamRef.current) {
          logServiceRef.current.log('error', '[HeadVerification] No stream available');
          resolve(false);
          return;
        }
        recorder = new MediaRecorder(streamRef.current, { mimeType: 'video/webm;codecs=vp9' });
      } catch (err) {
        logServiceRef.current.log('error', `[HeadVerification] Failed to create MediaRecorder: ${err}`);
        resolve(false);
        return;
      }
      ...
      recorder.onstop = () => {
        ...
        if (success) {
          setVerificationMessage('✅ Head movement verified — downloading...');
          logServiceRef.current.log('info', '[HeadVerification] 🎥 Recording complete. Starting download.');
          showMessage('headTurnAttemptStatus', '✅ Head movement verified — downloading...');
          resolve(true);
        } else {
          logServiceRef.current.log('info', '[HeadVerification] 🎥 Recording complete. No verification.');
          resolve(false);
        }
      };
      ...
      logServiceRef.current.log('info', `[HeadVerification] Started. Direction: ${direction}, YawThreshold: ${yawThreshold}, PitchThreshold: ${pitchThreshold}`);
      showMessage('headTurnAttemptStatus', `[HeadVerification] Started. Direction: ${direction}, YawThreshold: ${yawThreshold}, PitchThreshold: ${pitchThreshold}`);
      ...
      try {
        ...
        logServiceRef.current.log('warn', `[HeadVerification] Could not clear overlay canvas: ${err}`);
      }
      ...
      const sampler = setInterval(async () => {
        try {
          ...
          if (!detections || !detections.landmarks) {
            logServiceRef.current.log('warn', '[HeadVerification] No face detected in current frame');
            return;
          }
          ...
          if (baselineYaw === null || baselinePitch === null) {
            ...
            logServiceRef.current.log('info', `[HeadVerification] Baseline yaw=${baselineYaw.toFixed(3)} pitch=${baselinePitch.toFixed(3)}`);
          ...
          if (dist > ovalRef.current.rOuter) {
            if (isRecordingVerification && recorder && recorder.state !== 'inactive') {
              logServiceRef.current.log('info', '[HeadVerification] User moved outside oval, stopping recording.');
              recorder.stop();
          ...
          logServiceRef.current.log('info', `[HeadVerification] ΔYaw=${yawDelta.toFixed(3)} ΔPitch=${pitchDelta.toFixed(3)}`);

          if (direction === 'forward') {
            // For 'forward' direction: just verify face stays centered and stable
            const isStable = Math.abs(yawDelta) < 0.03 && Math.abs(pitchDelta) < 0.03;
            if (!isRecordingVerification) {
              recorder?.start();
              isRecordingVerification = true;
              showMessage('headTurnAttemptStatus', `Capturing face — keep looking STRAIGHT…`);
            }
            if (isStable) {
              isVerified = true;
              success = true;
              ...
              setVerificationMessage(`📷 Keep your face STRAIGHT and STILL…`);
            }
          } else if (direction === 'left' || direction === 'right') {
            const passes = (direction === 'left' && yawDelta < -yawDeltaThreshold) || (direction === 'right' && yawDelta > yawDeltaThreshold);
            if (!isRecordingVerification && Math.abs(yawDelta) > yawDeltaThreshold / 2) {
              recorder?.start();
              isRecordingVerification = true;
              showMessage('headTurnAttemptStatus', `Capturing head turn (${direction.toUpperCase()})…`);
            }
            if (isRecordingVerification && Math.abs(yawDelta) < yawDeltaThreshold / 3) {
              recorder?.stop();
              isRecordingVerification = false;
            }
            if (passes) {
              isVerified = true;
              success = true;
              setVerificationMessage(`✅ ${direction.toUpperCase()} movement detected.`);
            } else {
              setVerificationMessage(`${direction === 'left' ? '⬅️' : '➡️'} Keep turning your head ${direction.toUpperCase()}.`);
            }
          } else if (direction === 'down' || direction === 'up') {
            const passes = (direction === 'down' && pitchDelta > pitchDeltaDownThreshold) || (direction === 'up' && pitchDelta < pitchDeltaUpThreshold);
            if (!isRecordingVerification && ((direction === 'down' && pitchDelta > pitchDeltaDownThreshold / 2) || (direction === 'up' && pitchDelta < pitchDeltaUpThreshold / 2))) {
              recorder?.start();
              isRecordingVerification = true;
              showMessage('headTurnAttemptStatus', `Capturing head movement (${direction.toUpperCase()})…`);
            }
            if (isRecordingVerification && ((direction === 'down' && pitchDelta < pitchDeltaDownThreshold / 3) || (direction === 'up' && pitchDelta > pitchDeltaUpThreshold / 3))) {
              recorder?.stop();
              isRecordingVerification = false;
            }
            if (passes) {
              ...
              setVerificationMessage(`${direction === 'down' ? '⬇️' : '⬆️'} Keep moving your head ${direction.toUpperCase()}.`);
            }
          }
          ...
          if (isVerified) {
            ...
            setVerificationMessage(`✅ VERIFIED direction (${direction.toUpperCase()})!`);
            showMessage('statusMessage', `✅ VERIFIED direction (${direction.toUpperCase()})!`);

            // Add glow effect to video element for feedback
            const videoEl = videoRef.current;
            if (videoEl) {
              videoEl.classList.add('selfie-glow-green');
              console.log('[HeadVerification] ✅ Added glow-green class to video');
              setTimeout(() => {
                videoEl.classList.remove('selfie-glow-green');
                console.log('[HeadVerification] Removed glow-green class from video');
              }, 3000);
            } else {
              console.warn('[HeadVerification] ⚠️ Could not find video element for glow effect');
            }
          }

        } catch (err) {
          logServiceRef.current.log('error', `[HeadVerification] Sampler error: ${err}`);
        }
      }, 150);
      ...
      setTimeout(() => {
        try {
          ...
          if (!success) {
            if (isRecordingVerification && recorder && recorder.state !== 'inactive') recorder.stop();
            setVerificationStatus(false);
            setHeadTurnRecordingFailed(true);
            setVerificationMessage(`❌ Head movement (${direction.toUpperCase()}) timed out. Please try again.`);
            logServiceRef.current.log('error', `[HeadVerification] ❌ TIMEOUT after ${HEAD_VERIFICATION_TIMEOUT_MS}ms. Direction: ${direction} not verified in time.`);
            resolve(false);
          }
        } catch (err) {
          logServiceRef.current.log('error', `[HeadVerification] Timeout cleanup failed: ${err}`);
          resolve(false);
        }
      }, HEAD_VERIFICATION_TIMEOUT_MS);

    } catch (err) {
      logServiceRef.current.log('error', `[HeadVerification] Unexpected error in verification: ${err}`);
      resolve(false);
    }
  });
}, [showMessage, startBlinking, stopBlinking, setHeadTurnBlob, setVerificationMessage, setVerificationStatus, setHeadTurnRecordingDone, setHeadTurnRecordingFailed]);


// Returns: true = success, false = explicit failed attempt, null = defer (camera not ready)
const startHeadMovementVerification = useCallback(async (
  direction: 'left' | 'right' | 'up' | 'down' | 'forward'
): Promise<boolean | null> => {
  try {
    // Check if the camera is active before starting verification
    if (sessionCompletedRef.current) {
      logServiceRef.current.log('info', '[HeadVerification] Ignored after session completion');
      return null;
    }
  ...
  isVerifyingHeadTurnRef.current = false;
    ...
    logServiceRef.current.log('error', `[HeadVerification] Unexpected error: ${outerErr}`);
...
const performVerificationForCurrentSegment: () => Promise<void> = useCallback(async () => {
  const segment = currentSegmentRef.current; // ✅ Use ref to avoid stale closure
  console.log('info', '[HeadVerification] performVerificationForCurrentSegment called for segment', segment);
  setShowHeadTurnPrompt(true);
  verificationPendingRef.current = true;

  let verificationDirection: 'up' | 'down' | 'left' | 'right' | 'forward';
  if (verificationInProgressRef.current) {
    logServiceRef.current.log('info', 'Head verification already in progress; skipping new request.');
  ...
  if (prePlanned) {
    ...
    else if (segment === 2) setSecondVerificationDirection(verificationDirection);
    ...
    verificationDirection = getRandomDirection([firstVerificationDirection!]);
    setSecondVerificationDirection(verificationDirection);
  } else {
    verificationDirection = getRandomDirection([firstVerificationDirection!, secondVerificationDirection!]);
  ...
  if (sessionCompletedRef.current) {
    logServiceRef.current.log('info', 'Head verification prompt suppressed after session completion.');
    verificationInProgressRef.current = false;
    return; // Do not proceed once session done
  }
  logServiceRef.current.log('info', `Prompting user to ${verificationDirection}`);
  ...
  console.log('info', '[HeadVerification] Checking stream readiness. streamRef.current=', !!streamRef.current);
  if (!streamRef.current) {
    const videoEl = videoRef.current;
    if (videoEl && videoEl.srcObject instanceof MediaStream) {
      streamRef.current = videoEl.srcObject as MediaStream;
      console.log('info', '[HeadVerification] Recovered stream from video element – proceeding');
    }
  }

  // If after recovery we STILL have no stream, defer and keep pending true to block next segment
  if (!streamRef.current) {
    console.log('warn', '[HeadVerification] Deferring – stream not ready (post-recovery)');
    verificationInProgressRef.current = false;
    ...
    setTimeout(() => {
      console.log('info', '[HeadVerification] Re-attempting after deferral...');
      if (!sessionCompletedRef.current) {
  ...
  console.log('info', '[HeadVerification] Stream ready, proceeding to create headMediaRecorder. Stream has', streamRef.current.getTracks().length, 'tracks');
  ...
  if (MediaRecorder.isTypeSupported('video/webm;codecs=vp9')) {
    ...
    logServiceRef.current.log('error', 'No supported MIME type found for MediaRecorder on this browser.');
  ...
  if (!streamRef.current) {
    showMessage('statusMessage', '⚠️ Camera stream not ready for head verification.');
    return;
  }
  ...
  headRecorder.onstop = () => {
    if (sessionCompletedRef.current) {
      logServiceRef.current.log('info', 'Head verification stop ignored after session completion.');
      return;
    }
  ...
  const success = await startHeadMovementVerification(verificationDirection);

  // Stop the headMediaRecorder when done verification
  if (headRecorder.state === 'recording' || headRecorder.state === 'paused') {
    headRecorder.stop();
  }
  ...
  if (success === true) {
    ...
    console.log('info', `[HeadVerification] ✅ SUCCESS for segment ${segment}. verificationDoneForSegmentRef is now:`, verificationDoneForSegmentRef.current);
    setHeadTurnAttempts(0);
    ...
    if (!sessionCompletedRef.current) {
      ...
      console.log('info', `[HeadVerification] Resuming segment ${segment} from ${resumeFrom}s (target: ${segmentTarget}s)`);
      console.log('info', `   This will trigger immediate completion and move to next segment`);
      recordingFlagRef.current = 1;
      // Resume the current segment from where it paused (use ref value for accuracy)
      setTimeout(() => _startSegmentRecording(segmentSecondsRecordedRef.current), 600);
    }
    verificationPendingRef.current = false;
    ...
    setHeadTurnAttemptsPerSegment(prev => ({ ...prev, [segment]: newAttempts }));
    showMessage('headTurnAttemptStatus', `❌ Head turn failed attempt ${newAttempts}. Please try again.`);
    ...
    if (newAttempts >= maxHeadTurnAttempts) {
      ...
      if (segment === 1) {
        setHeadTurnRecordingFailed(true);
        showMessage('headTurnAttemptStatus', `❌ Verification 1 failed ${newAttempts} times. Restarting all segments.`);
        _resetAll();
        setTimeout(() => _startSegmentRecording(0), 1000);
      } else if (segment === 2) {
        setHeadTurnRecordingFailed(true);
        showMessage('headTurnAttemptStatus', `❌ Verification 2 failed. Will trigger verification 3 after final video.`);
    ...
    logServiceRef.current.log('info', '[HeadVerification] Deferred; will retry when camera active');
    setTimeout(() => {
      if (isCameraOn && !sessionCompletedRef.current) performVerificationForCurrentSegment();
    }, 1500);
    // pending stays true
  }
...
const downloadAllBlobs = useCallback(() => {
  if (downloadsTriggeredRef.current) {
    console.log('info', 'Downloads already triggered; skipping duplicate call.');
    return;
  }
...
const onSegmentComplete = useCallback(async () => {
  ...
  if (sessionCompletedRef.current) {
    console.log('info', '[onSegmentComplete] Ignored because sessionCompletedRef is true');
    return;
  }
  ...
  if (seg < totalSegments) {
    ...
    console.log('info', `[onSegmentComplete] Final segment. triggerVerification3=${triggerVerification3}, segment3Done=${segment3Done}`);
    if (triggerVerification3 && !segment3Done) {
      // Segment 2 verification failed, need to do verification 3
      console.log('info', '[onSegmentComplete] Segment 2 verification failed, triggering verification 3');
      await performVerificationForCurrentSegment();
    } else {
      console.log('info', '[onSegmentComplete] All segments and verifications complete! Completing session.');
...
const shouldVerifyAfterSegment = useCallback((segmentNumber: number): boolean => {
  console.log('info', `[shouldVerifyAfterSegment] Called for segment ${segmentNumber}`);
  // Segments 1 and 2 always get verification after completion
  if (segmentNumber === 1 || segmentNumber === 2) {
    const alreadyDone = !!verificationDoneForSegmentRef.current?.[segmentNumber];
    const decision = !alreadyDone;
    console.log('info', `[shouldVerifyAfterSegment] Segment ${segmentNumber} done=${alreadyDone} -> return ${decision}`);
    return decision;
  }
  // Segment 3 only gets verification if segment 2's verification FAILED
  if (segmentNumber === 3) {
    const segment2Failed = verificationSuccessForSegmentRef.current?.[2] === false;
    console.log('info', `[shouldVerifyAfterSegment] Segment 3: segment2Failed=${segment2Failed} -> return ${segment2Failed}`);
    return segment2Failed;
  }
  console.log('info', `[shouldVerifyAfterSegment] Segment ${segmentNumber} -> returning FALSE`);
  return false;
}, []);



const _startSegmentRecording: (resumeSecondsRecorded?: number) => Promise<void> = useCallback(async (resumeSecondsRecorded = 0) => {
  if (startingSegmentRef.current) {
    console.log('warn', 'Start already in progress - ignoring duplicate call.');
    return;
  }
  ...
  if (sessionCompletedRef.current || autoStartDisabledRef.current) {
    console.log('info', '[startSegment] Aborting start because sessionCompletedRef or autoStartDisabledRef is true');
    return;
  }
  ...
  try {
    ...
    if (resumeSecondsRecorded === 0) {
      ...
      if (MediaRecorder.isTypeSupported('video/webm;codecs=vp9')) {
        ...
        console.log('info', 'No supported MIME type found for MediaRecorder on this browser.');
        showMessage('statusMessage', '⚠️ MediaRecorder MIME type not supported.');
        return;
      }
    } else {
    ...
    if (resumeSecondsRecorded === 0 && !isRecording) {
      ...
      if (segment === 1 && (segmentSecondsRecorded === undefined || segmentSecondsRecorded === null)) {
        setCompletedSegments([]);
        setVerificationDoneForSegment({});
        setHeadTurnAttemptsPerSegment({});
        setHeadVerificationCountPerSegment({});
      ...
      setVerificationDoneForSegment(prev => ({ ...prev, [segment]: false }));
      verificationDoneForSegmentRef.current[segment] = false; // ✅ Sync ref
      setHeadTurnAttemptsPerSegment(prev => ({ ...prev, [segment]: 0 }));
      setHeadVerificationCountPerSegment(prev => ({ ...prev, [segment]: 0 }));
    } else {
  ...
  console.log('info', 'MediaRecorder created', { state: recorder.state, options });

    const segmentTarget = segmentDurations[segment - 1] || totalDuration / totalSegments;
    setTimeRemaining(segmentTarget - segmentSecondsRecorded);
    
    if (!segmentDurations || segmentDurations.length === 0) {
      console.log('warn', 'Segment durations not initialized yet, using default values');
    }

    if (segmentTarget - segmentSecondsRecorded <= 0) {
      console.log('info', 'Segment duration already met or exceeded, calling _onSegmentComplete.');
    ...
    recorder.ondataavailable = (e) => {
      if (e.data && e.data.size > 0) {
        console.log('info', `📦 ondataavailable: received chunk of size ${e.data.size} bytes`);
        // Save chunks only when exactly one matching face is inside the oval
        if (isFaceDetectedRef.current && !multipleFacesDetectedRef.current && !differentFaceDetectedRef.current) {
    ...
    recorder.onstop = async () => {
      ...
      const chunkCount = recordedChunksPerSegmentRef.current[segment]?.length || 0;
  ...
  console.log('info', `📹 onstop: wasFaceDetected(ref)=${isFaceDetectedRef.current}, isSegmentValid=${isSegmentValid}, hasEnoughTime=${hasEnoughTime} (${actualSecondsRecorded}>=${segmentTarget}), hasValidChunks=${hasValidChunks} (${chunkCount} chunks)`);
      ...
      if (stoppingForRestartRef.current) {
        ...
        const sessionDuration = actualSecondsRecorded - currentSessionStartTimeRef.current;
      ...
      if (isSegmentValid && hasEnoughTime && hasValidChunks) {
        ...
        console.log('info', `✅ Segment ${segment} COMPLETED and saved. Blob size: ${blob.size} bytes, Chunk count: ${chunks.length}`);
        console.log('info', `   Segment recorded for ${actualSecondsRecorded}s (target: ${segmentTarget}s)`);
        ...
        console.log('warn', `❌ Segment ${segment} incomplete; retrying. (face=${wasFaceDetected}, valid=${isSegmentValid}, time=${hasEnoughTime}, chunks=${hasValidChunks})`);
        setTimeout(() => _startSegmentRecording(actualSecondsRecorded), 600);
        return;
      }

      console.log('info', `[onstop] About to check shouldVerifyAfterSegment(${segment})`);
      const shouldVerify = shouldVerifyAfterSegment(segment);
      console.log('info', `[onstop] shouldVerifyAfterSegment(${segment}) returned:`, shouldVerify);
      
      if (shouldVerify) {
        console.log('info', `[onstop] Calling performVerificationForCurrentSegment for segment ${segment}`);
        await performVerificationForCurrentSegment();
    ...
    timerIntervalRef.current = setInterval(async () => {
      ...
      if (multipleFacesDetectedRef.current || differentFaceDetectedRef.current) {
        if (recorder.state === 'recording') {
          recorder.pause();
          // Silent pause - no UI message for background recording
      ...
      if (recorder.state === 'recording') {
        ...
        console.log('info', `⏱️ Timer tick: currentSeconds=${currentSeconds}, target=${segmentTarget}`);
        ...
        if (currentSeconds < segmentTarget) {
          ...
          if (await checkDifferentFace()) {
            ...
            showMessage('verificationMessage', '❌ Different face detected for several seconds! Restarting from scratch...');
            clearInterval(timerIntervalRef.current);
            _resetAll();
          ...
          if (currentSeconds !== segmentTarget && extraSeconds < 1) {
            ...
            console.log('info', `⏱️ Adding extra second buffer for timing mismatch (extra=${extraSeconds + 1})`);
            return;
          }
          console.log('info', `✅ Segment complete at ${currentSeconds}s (target: ${segmentTarget}s, extra=${extraSeconds}), stopping recorder`);
...
const _restartCurrentSegmentDueToFaceLoss: () => void = useCallback(() => {
  console.log('info', 'Attempting to restart current segment due to face loss.');
  ...
  if (mediaRecorderRef.current && mediaRecorderRef.current.state === 'recording') {
    ...
    showMessage('verificationMessage', "⚠️ Recording reset due to face loss. Continuing from current progress...");
    console.log('warn', 'Recording reset due to face loss or quality issues.');

    clearInterval(timerIntervalRef.current);
    console.log('info', 'Timer interval cleared.');

    try {
      console.log('info', `Stopping mediaRecorder. Current state: ${mediaRecorderRef.current.state}`);
      mediaRecorderRef.current.stop();
      console.log('info', 'MediaRecorder.stop() called successfully.');
    } catch (stopErr) {
      console.log('error', `Error stopping mediaRecorder during restart: ${stopErr}`);
    }
  } else {
    console.log('info', 'MediaRecorder not recording or undefined, skipping stop.');
  }
  ...
  setTimeout(() => {
    ...
    console.log('info', `[Restart] Cleared verification flags for segment ${segment}, reset face mismatch counter`);
    ...
    if (
      ...
      console.log('info', `Adjusted segmentSecondsRecorded by -1 for segment ${segment}`);
    } else {
      // Otherwise, use the segmentSeconds as is (or 0)
      resumeTime = segmentSeconds;
    }

    console.log('info', `Resuming recording from ${resumeTime}s for segment ${segment}.`);

    if (!mediaRecorderRef.current || mediaRecorderRef.current.state !== 'recording') {
      console.log('info', 'Starting segment recording after restart.');
      _startSegmentRecording(resumeTime);
    } else {
      console.log('warn', 'Attempted to restart recording but MediaRecorder is already recording.');
    }
  }, 1000);
...
const showAndLogMessage = useCallback((
  ...
  scheduleNext(loop);
  return true;
}, [showMessage, isRecording, _restartCurrentSegmentDueToFaceLoss, scheduleNext]);


const checkMultipleFaces = useCallback(async (
  loop: () => void,
  options: any
): Promise<boolean> => {
  ...
  return false;
}, [isRecording, isVerifyingHeadTurn, _restartCurrentSegmentDueToFaceLoss, showAndLogMessage, showMessage]);
...
const getCurrentFrameAsMat = useCallback((): any => {
  ...
  try {
    ...
    const mainRadius = ovalRef.current?.rInner || Math.max(Math.min(centerX, centerY) - 10, 1);
...
const handleBrightnessChecks = useCallback(async (loop: () => void): Promise<boolean> => {
  const brightness = getFrameBrightness();
  console.log('info', `[BrightnessCheck] Frame brightness: ${brightness}`, 'brightness');
  ...
  if (isVideoBlank()) {
    ...
    console.log('warn', '[BrightnessCheck] Camera feed appears blank.', 'cameraError');
    return showAndLogMessage(
      'cameraErrorMessage',
  ...
  if (mat) {
    ...
    console.log('info', `[BrightnessCheck] Bright spot detection result: ${result}`, 'brightness');
    mat.delete();
    ...
    if (!openCvFrameWarningShownRef.current) {
      console.log('warn', '[BrightnessCheck] Failed to get current frame as Mat (OpenCV not ready). Suppressing further warnings.', 'cameraError');
  ...
  if (showWarning) {
    ...
    console.log('warn', '[BrightnessCheck] Bright spot detected in the oval.', 'brightnessWarning');
    return showAndLogMessage(
      'brightnessMessage',
  ...
  if (brightness < 60) {
    ...
    console.log('warn', '[BrightnessCheck] Too dark environment detected.', 'brightnessWarning');
    return showAndLogMessage(
      'brightnessMessage',
    ...
    console.log('warn', '[BrightnessCheck] Too bright environment detected.', 'brightnessWarning');
    return showAndLogMessage(
      'brightnessMessage',
    ...
    console.log('info', '[BrightnessCheck] Brightness is within acceptable range.', 'brightness');
  ...
  if (isVideoBlurred()) {
    ...
    console.log('warn', '[BrightnessCheck] Video is blurry detected.', 'blurWarning');
    return showAndLogMessage(
      'dashedCircleAlignMessage',
  ...
  console.log('info', '[BrightnessCheck] No issues detected in current frame.', 'status');
  return false;
}, [
  getFrameBrightness,
  drawFaceGuideOverlay,
...
const detectSingleFaceWithLandmarks = useCallback(async (options: any): Promise<void> => {
...
const handleFaceAlignment = useCallback(
  async (_loop: () => void): Promise<[boolean, boolean]> => {
    ...
    const smoothedFill =
      fillBufferRef.current.reduce((a, b) => a + b, 0) / fillBufferRef.current.length;
    ...
    if (smoothedFill < lowerBound) {
      ...
      showMessage('distanceMessage', '📏 Please move slightly farther away from the camera.');
    } else {
      sizeOK = true;
...
const startRecording_FaceReference = useCallback(async (): Promise<void> => {
  ...
  if (!isFaceDetectedRef.current) {
    showMessage('statusMessage', '🙋 Please align your face inside the circle and adjust distance before starting.');
  ...
  try {
    ...
    const videoEl = videoRef.current;
    if (!videoEl) {
      console.log('warn', 'Video element not ready while capturing reference face.');
    } else {
      const detection = await faceapi
        .detectSingleFace(videoEl, new faceapi.TinyFaceDetectorOptions())
        .withFaceLandmarks()
      ...
      if (detection && detection.descriptor) {
        ...
        console.log('warn', '⚠️ Face not detected clearly; continuing without reference descriptor');
        showMessage('statusMessage', '⚠️ Face not detected clearly. Please adjust position.');
      }
    }
  } catch (err: any) {
    // Handle any errors thrown by FaceAPI and notify user
    console.log('error', `❌ FaceAPI error while capturing reference face: ${err?.message || err}`);
    showMessage('statusMessage', '⚠️ Error detecting face, continuing with recording.');
  }
...
const handleNoFaceDetected = useCallback((loop: () => void): void => {
    ...
    if (lastBoxRef.current && lastLandmarksRef.current) {
      ...
      if (sizeOK && faceInside) {
        ...
        if (insideOvalFramesRef.current >= requiredFrames) {
          ...
          const recorderActive = mediaRecorderRef.current && 
                                 (mediaRecorderRef.current.state === 'recording' || 
                                  mediaRecorderRef.current.state === 'paused');
          
          console.log('info', `Recording flag: ${recordingFlagRef.current}, isRecording: ${isRecording}, recorderState: ${mediaRecorderRef.current?.state || 'none'}`);
          // Use ref-based isRecording for more accurate immediate state
          console.log('debug', `Ref states -> isRecordingRef=${isRecordingRef.current}, startingGuard=${startingSegmentRef.current}`);
          console.log('info', `Recording flag: ${recordingFlagRef.current}, isRecording(state)=${isRecording}, isRecording(ref)=${isRecordingRef.current}, recorderState: ${mediaRecorderRef.current?.state || 'none'}`);
          ...
          if (processingSegmentCompletionRef.current) {
            console.log('info', '⏳ Processing segment completion, not starting new recording');
            return;
          }
          ...
          if (recordingFlagRef.current === 0 && !recorderActive) {
                        ...
                        if (
                          ...
                          console.log('info', 'Deferring segment start. Flags:', {
                            ...
                            processingSegmentCompletion: processingSegmentCompletionRef.current,
                  ...
                  if (segToStart === 1) {
                    ...
                    chosen = ((): 'up' | 'down' | 'left' | 'right' => {
                      const baseExclude = [firstVerificationDirection!].filter(Boolean) as any;
                    ...
                    const exclude: any[] = [];
                    if (firstVerificationDirection) exclude.push(firstVerificationDirection);
                    if (secondVerificationDirection) exclude.push(secondVerificationDirection);
                    chosen = ((): 'up' | 'down' | 'left' | 'right' => {
                  ...
                  showMessage('headTurnAttemptStatus', `Please ${chosen === 'left' ? 'turn your head LEFT' : chosen === 'right' ? 'turn your head RIGHT' : chosen === 'up' ? 'tilt your head UP' : 'tilt your head DOWN'} and KEEP it there — recording will start now.`);
                  ...
                  setTimeout(async () => {
                    ...
                    try {
                      ...
                      console.log('error', `❌ Error starting segment recording: ${err}`);
                      recordingFlagRef.current = 0;
                    } finally {
        ...
        if (!sizeOK && !faceInside) {
          console.log('dashedCircleAlignMessage', '🧭 Make sure your full face is inside the dashed circle and adjust distance.');
        } else if (!faceInside) {
          console.log('dashedCircleAlignMessage', '🧭 Your entire face must be inside the dashed circle.');
        } else {
...
const startCamera = useCallback(async () => {
  try {
    ...
    if (videoTracks.length) {
      ...
      if ((settings.width && settings.width < 400) || (settings.height && settings.height < 300)) {
        setCameraErrorMessage('⚠️ Low camera resolution. Face detection may not work properly.');
        // logService.log('warn', 'Low camera resolution detected.');
      }

      if (settings.frameRate && settings.frameRate < 15) {
        setCameraErrorMessage('⚠️ Low frame rate. Detection quality may be affected.');
    ...
    if (fps < 10) {
      setCameraErrorMessage('⚠️ Camera FPS is too low for reliable detection.');
  ...
  const ctx = brightnessCanvasRef.current.getContext('2d', { willReadFrequently: true } as any);
...
const checkVideoResolution = useCallback(() => {
  ...
  if (width < 400 || height < 300) {
    console.log('cameraErrorMessage', "⚠️ Low camera resolution. Face detection may not work properly.");
  }
}, []);
  ...
  <div className="selfie-app-root">
      ...
      {isVerifyingHeadTurn && (
        <h3 className="head-turn-instruction">
          👤 Move your head toward the <span style={{ color: 'rgb(35, 27, 148)' }}>blinking arc</span> and hold
        </h3>
      )}
          ...
          {isRecording && (
            <div className="selfie-recording-panel">
              <div className="info">
                ...
                {isCameraOn && !isRecording && completedSegments.length < totalSegments && (
                  <p>
                    ✔ Ready – Keep your face inside the oval (75–85% height), then hit <strong>Start</strong>.
                  </p>
                )}
              </div>
              <p>⏺️ Recording </p>
      ...
      {isMobile && mobileStatusMessage && (
        <div className="selfie-mobile-status-message">{mobileStatusMessage}</div>
      )}
